...
class CubesTester(unittest.TestCase):
    ...
    def test_fit_curve(self):
        ...
        rang = [np.cos(rang) + 0.5 * np.sin(rang) + np.random.rand(24) * 0.1, np.cos(rang) + 0.5 * np.sin(rang) + np.random.rand(24) * 0.2] # define data with y = 0 + 1 * cos() + 0.5 *sin()
        xdata = xr.DataArray(rang, coords=[["NY", "LA"], pd.date_range("2000-01-01", periods=24, freq='M')], dims=["space", "time"])
        def func(x, a, b, c):
